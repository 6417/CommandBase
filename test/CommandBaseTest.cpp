...
TEST_F(CommandBaseTest, SchedulerCallsMultipleCommandsCorrectly)
{
    ...
    for (auto& command : commands)
        fridolinsRobotik::CommandScheduler::getInstance().schedule(make_stack_pointer<TestCommand>(&command));
    fridolinsRobotik::CommandScheduler::getInstance().run();
...
{
    std::shared_ptr<fridolinsRobotik::CommandBase> testCommandPtr;
    {
        TestCommand testCommandInScope{};
        testCommandPtr = std::shared_ptr<fridolinsRobotik::CommandBase>(&testCommandInScope, [](fridolinsRobotik::CommandBase*) {});
        fridolinsRobotik::CommandScheduler::getInstance().schedule(make_stack_pointer<TestCommand>(&testCommandInScope));
        fridolinsRobotik::CommandScheduler::getInstance().run();
        ASSERT_TRUE(fridolinsRobotik::CommandScheduler::getInstance().isRunning(make_stack_pointer<TestCommand>(&testCommandInScope)));
    }
...
TEST_F(CommandBaseTest, CommandSchedulerCancelAll_WillCancelAllCommandCancelCommandsCorrectly)
{
    ...
    ASSERT_FALSE(fridolinsRobotik::CommandScheduler::getInstance().isRunning(testCommand));
    ASSERT_FALSE(fridolinsRobotik::CommandScheduler::getInstance().hasBeenInitialized(testCommand));
    ASSERT_FALSE(fridolinsRobotik::CommandScheduler::getInstance().isRunning(make_stack_pointer<TestCommand>(&testCommand1)));
    ASSERT_FALSE(fridolinsRobotik::CommandScheduler::getInstance().hasBeenInitialized(make_stack_pointer<TestCommand>(&testCommand1)));

    fridolinsRobotik::CommandScheduler::getInstance().schedule(testCommand);
    fridolinsRobotik::CommandScheduler::getInstance().schedule(make_stack_pointer<TestCommand>(&testCommand1));
    ASSERT_TRUE(fridolinsRobotik::CommandScheduler::getInstance().isRunning(testCommand));
    ASSERT_FALSE(fridolinsRobotik::CommandScheduler::getInstance().hasBeenInitialized(testCommand));
    ASSERT_TRUE(fridolinsRobotik::CommandScheduler::getInstance().isRunning(make_stack_pointer<TestCommand>(&testCommand1)));
    ASSERT_FALSE(fridolinsRobotik::CommandScheduler::getInstance().hasBeenInitialized(make_stack_pointer<TestCommand>(&testCommand1)));
    ...
    ASSERT_TRUE(fridolinsRobotik::CommandScheduler::getInstance().hasBeenInitialized(testCommand));
    ASSERT_TRUE(fridolinsRobotik::CommandScheduler::getInstance().isRunning(make_stack_pointer<TestCommand>(&testCommand1)));
    ASSERT_TRUE(fridolinsRobotik::CommandScheduler::getInstance().hasBeenInitialized(make_stack_pointer<TestCommand>(&testCommand1)));
